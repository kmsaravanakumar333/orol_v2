...
class WaterTestDetails {
  ...
  Future<WaterTestDetails> createWaterTestDetail(waterTestObj, riverPictures, surroundingPictures, context) async {
    ...
    // Add the other fields to the request body
    //utf8.encode(json.encode(data))
    // jsonEncode(surroundingPictures).toString()
    // request.fields['generalInformation'] = json.encode(waterTestObj.value['generalInformation']);
    // request.fields['waterLevelAndWeather'] = json.encode(waterTestObj.value['waterLevelAndWeather']);
    // request.fields.addAll((json.encode(waterTestObj)));
    request.fields['userId'] = "${user.id}";
    request.fields['generalInformation'] = jsonEncode(waterTestObj.value['generalInformation']).toString();
    request.fields['waterLevelAndWeather'] = jsonEncode(waterTestObj.value['waterLevelAndWeather']).toString();
    request.fields['surroundings'] = jsonEncode(waterTestObj.value['surroundings']);
    request.fields['waterTesting'] = jsonEncode(waterTestObj.value['waterTesting']).toString();
    request.fields['floraPictures'] = jsonEncode(waterTestObj.value['floraPictures']);
    request.fields['faunaPictures'] = jsonEncode(waterTestObj.value['faunaPictures']);
    request.fields['artworkPictures'] = jsonEncode(waterTestObj.value['artworkPictures']);
    request.fields['groupPictures'] = jsonEncode(waterTestObj.value['groupPictures']);
    request.fields['activityPictures'] = jsonEncode(waterTestObj.value['activityPictures']);
    request.fields['riverPictures'] = jsonEncode(waterTestObj.value['riverPictures']);
    request.fields['surroundingPictures'] = jsonEncode(waterTestObj.value['surroundingPictures']);
    ...
    if (response.statusCode == 201) {
      ...
      ScaffoldMessenger.of(context!).showSnackBar(const SnackBar(content: Text("Failed to create user")));
